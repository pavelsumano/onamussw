...
class Library extends Component {
  ...
  renderSingleBook(book) {
    return (
      <div>
        <h1>Visit Book Detail</h1>
        <h2>{book.visita}</h2>
        <p>Asunto: {book.asunto}</p>
        <p>Entrada:{book.entrada}</p>
        <p>Salida: {book.salida}</p>
        <p><img src={book.image} style={{ maxWidth: '100px' }} /></p>
  ...
  renderBooksList(books) {
    return (
      <div>
        <h1>Visit Book</h1>
        <p>A quien Visita - Asunto</p>
        <ul>
          {
            books.map((book, key) => {
              return (
                <li key={key}>
                  <Link to={`/library/${book.id}`}>{book.visita}</Link> - {book.asunto}
                </li>
              )
            })
          }
